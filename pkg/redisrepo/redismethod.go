...
func RegisterNewUser(username, password string) error {
	// redis-cli
	// syntax: SET key value, only unique values, returns error when item exists...
...
func UpdateContactList(username, contact string) error {
	...
	// redis-cli
	// syntax: ZADD key SCORE MEMBER, uses sortedset datatype.
...
func CreateChat(c *model.Chat) (string, error) {
	...
	if err != nil {
		log.Println("error while updating contact lost of", c.From)
	}

	err = UpdateContactList(c.To, c.From)
	if err != nil {
		log.Println("error while updating contact lost of", c.To)
	}

	return chatKey, nil
}

// CreateFetchChatBetweenIndex creates index that will be used to understand the query and search in the dataset
